...
Hamster Bin 是一个面向电子爱好者、实验室和小型团队的电子元件库存管理系统。它用 Go 后端、SQLite 数据库和 React 前端组成，支持本地运行，也支持将前端静态资源嵌入到单个 Go 二进制文件中部署。
...
- 库存流水：记录入库、出库、补录价格、撤销和冲销，保留库存变动原因。
...
项目仍在持续迭代中，适合个人库存、实验室轻量管理和自托管使用。当前默认使用 SQLite，未内置多用户权限模型；如需公网部署，建议启用 HTTPS、强密码和足够随机的 `JWT_SECRET`。
