...
- `POST /api/v1/components/:id/stock` 请求体为 `{ "amount": 10, "reason": "采购", "total_price_cents": 1234 }`；`amount` 正数为入库、负数为出库。入库且 `total_price_cents > 0` 时写入分摊单价与总价到流水，并更新元件 `unit_price_cents`；出库无需传价，若元件有参考单价则自动写入出库成本（参考单价 × 数量）到流水。库存更新与流水写入在同一事务中完成。
