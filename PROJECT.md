...
- `web/src/pages/Components.tsx` 是元件管理主页面，负责元件列表、分页搜索、分类筛选、供应商输入/自动创建、供应商料号录入、平台编码导入、可选 AI 解析、二维码录入、图片上传和库存变更入口。
...
- `POST /api/v1/components/parse` 请求体为 `{ "code": "...", "use_llm": false }`，`use_llm` 可省略且默认 false；仅嘉立创/LCSC 解析器会响应该选项。
