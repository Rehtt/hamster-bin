...
- `web/src/pages/Components.tsx` 是元件管理主页面，负责元件列表、分页搜索、分类筛选、元件编号录入/展示、厂家型号录入/展示、一键为未编号元件自动补号、供应商输入/自动创建、供应商料号录入、封装/位置/供应商历史下拉选项、平台编码导入、解析结果分类填充、可选 AI 解析（平台编码与扫码共用）、二维码录入、图片上传和库存变更入口。新增元件时可输入采购总价（元），前端换算为分提交并按库存数量展示分摊单价；入库弹窗同样支持总价录入。列表支持显示总数、切换每页条数、多选元件并批量修改存放位置（批量位置弹窗同样支持历史位置下拉）。
...
- `Component.unit_price_cents` 表示参考单价，单位为分；入库或新增元件带价格时会更新为最近一次分摊单价。
- `StockLog.unit_price_cents` 和 `StockLog.total_price_cents` 分别表示该条库存记录的分摊单价与录入总价，单位为分；出库记录默认不填价格。
- 金额在接口和数据库中统一使用整数分，前端展示时格式化为元；分摊规则为 `unit_price_cents = total_price_cents / quantity`（整数除法，余数不做逐件分摊）。
...
- 元件列表搜索会匹配元件编号、元件名称、厂家型号、制造商、参数值、供应商料号、描述和供应商名称；修改搜索字段时需同步检查 `ComponentRepository.GetAll` 和元件管理页搜索文案。
...
- `POST /api/v1/components/:id/stock` 请求体为 `{ "amount": 10, "reason": "采购", "total_price_cents": 1234 }`；`amount` 正数为入库、负数为出库。入库且 `total_price_cents > 0` 时写入分摊单价与总价到流水，并更新元件 `unit_price_cents`；出库不要求价格。库存更新与流水写入在同一事务中完成。
